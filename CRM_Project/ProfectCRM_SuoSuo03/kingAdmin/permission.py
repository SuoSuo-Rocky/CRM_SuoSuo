#!/usr/bin/env python
# -*- coding: utf-8 -*-#
# __name__   = ProfectCRM_SuoSuo03.permission.py 
# __date__   = 2019/6/19  18:57
# __author__ ='Shi Wei'
# __description__: 
# --------------------------------------------------------------
...
def perm_check(*args, **kwargs):
    ...
    for permission_key, permission_val in perm_dic.items():
        # permission_val # 列表
        per_url_name = permission_val[0]  # url_name
        per_method = permission_val[1]   # 请求 方法
        perm_args = permission_val[4]     # URL 关键字参数 --- 列表
        perm_kwargs = permission_val[5]   # URL 锚 参数    --- 字典
        # 钩子 函数 Hook
        custom_perm_func = None if len(permission_val) == 6 else permission_val[6]
